...
// Supabase-authoritative admin analytics. Never report stale process-local seed data in production.
adminProductionHardeningRouter.get('/analytics', async (_req, res) => {
  try {
    ...
    const revenue = rows
      .filter((o: any) => o.status !== 'CANCELLED' && o.payment_status !== 'REFUNDED')
      .reduce((sum: number, o: any) => sum + Number(o.total || 0), 0);
    const pendingOrders = rows.filter((o: any) => !['DELIVERED', 'CANCELLED'].includes(o.status)).length;
    const deliveredOrders = rows.filter((o: any) => o.status === 'DELIVERED').length;
    const cancelledOrders = rows.filter((o: any) => o.status === 'CANCELLED').length;
...
// Optimistic-concurrency protected order status transition.
// The status read is included in the update predicate so two admins cannot both advance
// the same order from the same previous state and overwrite each other's history.
