...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const fillDemoAdmin = () => {
    setEmail('admin@megacity.co.ke');
    setPassword('Admin@MegaCity2026');
    setTab('login');
  };
      ...
      <div className="relative bg-[#09090b] border border-zinc-800 text-zinc-100 rounded-lg w-full max-w-md p-5 sm:p-6 shadow-2xl animate-fadeIn">
        ...
        <div className="text-center mb-4">
          ...
          <p className="text-[11px] text-zinc-400 mt-0.5">
            {tab === 'login'
              ? 'Access orders, saved addresses, and wishlist'
              : 'Sign up for order tracking and expedited checkout'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3 text-left">
          {tab === 'register' && (
            <>
              ...
              <div>
                <label className="text-[11px] font-mono text-zinc-400 block mb-0.5">Phone Number (M-Pesa / Call)</label>
                <div className="relative">
          ...
          <div>
            <label className="text-[11px] font-mono text-zinc-400 block mb-0.5">Password</label>
            <div className="relative">
        ...
        <div className="mt-4 pt-3 border-t border-zinc-800 text-center">
          <span className="text-[10px] font-mono text-zinc-500 block mb-1.5">DEMO CREDENTIALS:</span>
          <div className="flex gap-2">
            <button
              type="button"
              onClick={fillDemoAdmin}
              className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-red-400 border border-red-900/40 text-[10px] font-mono font-bold py-1.5 px-2 rounded transition"
            >
              Fill Admin
            </button>
            <button
              ...
              className="flex-1 bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border border-zinc-800 text-[10px] font-mono py-1.5 px-2 rounded transition"
            >
              Fill Customer
            </button>
          </div>
